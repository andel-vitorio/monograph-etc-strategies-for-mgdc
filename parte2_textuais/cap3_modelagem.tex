...
\begin{table}[H]
  ...
  \begin{tabular}{cccc}
    ...
    Carga de Resistência Constante & $r_{C}$ & 10,00 & $\Omega$ \\
    Indutância & $L$ & 10,00 & mH \\
    Capacitância  & $C$ & 470 & $\mu$C \\
    \bottomrule
  \end{tabular}
...
\begin{table}[H]
  ...
  \begin{tabular}{cccccc}
    ...
    \cmidrule{3-4}
     &  & Buck 1 & Buck 2 &  \\
    \midrule
    Potência da \acrshort{cpl} & ${P_{\mathrm{cpl}}}_o$ & 25,00  & 100,00 & W \\
    Tensão do capacitor & ${v_C}_{\mathrm{o}}$ & 28,50 & 28,00  & V \\
    Corrente do indutor & ${i_L}_{\mathrm{o}}$ & 3,69 & 6,37 & A \\
...
Por fim, o período de simulação é de 0 a 50 milissegundos. No primeiro modelo, a variação da corrente do indutor no instante inicial $\delta i_{L}(0)$ é de $-0,32 \textrm{ A}$  e a variação da tensão do capacitor $\delta v_{C}(0)$ é de $-1,40 \textrm{ V}$. No segundo modelo,  $\delta i_{L}(0) = -0,18 \textrm{ A}$ e  $\delta v_{C}(0) = -1,40 \textrm{ V}$.

\subsubsection{Sinais de Entrada Constantes}

A primeira simulação consiste em manter os parâmetros de entrada do sistema constantes. Para isso, definiu-se que as entradas do sistema seriam exatamente iguais aos seus valores no ponto de operação. Portanto, a variação em torno do \acrshort{po} é nula, logo, $\delta d(t) = 0$ e $\delta P_{\mathrm{cpl}}(t) = 0 \,\mathrm{W}$. Os resultados obtidos nesta simulação do primeiro modelo do conversor Buck está apresentado na \autoref{fig:simulation_1_buck_1} e, do segundo modelo, na \autoref{fig:simulation_1_buck_2}.

Na \autoref{fig:simulation_1_buck_1}, observa-se que o modelo apresenta comportamento estável no ponto de operação. A corrente do indutor e a tensão do capacitor convergem para seus valores no ponto de operação em um curto período de tempo. Não há divergência significativa entre o modelo não linear e o modelo linearizado, indicando a precisão do modelo linearizado para representar o comportamento do sistema. 
...
No segundo modelo, no entanto, tanto a corrente do indutor quanto a tensão do capacitor apresentam um aumento gradativo e não se estabilizam, conforme mostrado na \autoref{fig:simulation_1_buck_2}. Portanto, o segundo modelo não converge para um estado estacionário, o que indica instabilidade. Ademais, ao comparar os modelos não lineares e os linearizados, observa-se que os gráficos da corrente do indutor e da tensão do capacitor apresentam diferenças visíveis a partir de $t = 0,04 \, \mathrm{s}$. Essas diferenças podem gerar uma divergência entre os valores desejados e os valores reais, o que deve ser considerado na análise do sistema em malha fechada.
