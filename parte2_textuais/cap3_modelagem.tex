...
\textcolor{red}{[Aqui não temos apenas uma CPL (que precisa estar melhor caracterizada), a figura e o texto fala de CRL+CPL. É bom esclarecer.]}
...
Nas simulações, os conversores Buck e Boost são avaliados em dois cenários distintos descritos abaixo:

\begin{itemize}
  \item \textbf{Cenário 1}: Neste cenário, os conversores Buck e Boost são simulados em duas situações distintas. No primeiro caso, o conversor Buck é simulado em torno do \acrshort{po} estável ($P_{\mathrm{o}, 1}$), enquanto o conversor Boost é simulado em torno do \acrshort{po} estável ($P_{\mathrm{o}, 3}$). No segundo caso, tanto o conversor Buck quanto o Boost são simulados em torno de um \acrshort{po} instável ($P_{\mathrm{o}, 2}$ para o Buck e $P_{\mathrm{o}, 4}$ para o Boost). Em cada caso, os parâmetros de entrada dos conversores são definidos com seus respectivos valores nos pontos de operação. O objetivo é avaliar a estabilidade e o desempenho dos conversores sob condições invariáveis, considerando o sinal de perturbação $P_{\mathrm{cpl}}(t)$ e a entrada controlável $d(t)$ fixos durante toda a simulação, analisando se eles mantêm sua operação dentro de limites aceitáveis. \label{cenario:1}
  \item \textbf{Cenário 2}: Neste cenário, os conversores Buck e Boost, em cada um dos pontos de operação $P_{\mathrm{o}, 1}$ e $P_{\mathrm{o}, 2}$, são submetidos a um sinal de perturbação $P_{\mathrm{cpl}}$ variável, ou seja, a potência da \acrshort{cpl} conectada ao conversor é alterada em determinados momentos. O objetivo é analisar como os conversores respondem à variações na \acrshort{cpl}. Ele permite observar como os sistemas se ajustam dinamicamente a estas mudanças e avaliar se o conversor é capaz de manter sua estabilidade e desempenho. \label{cenario:2}
\end{itemize}
...
No segundo cenário, foi avaliado o impacto do sinal de perturbação $P_{\mathrm{cpl}}$ no conversor Buck, variando a potência da \acrshort{cpl}. A \autoref{fig:power_cpl_variation} apresenta os valores do sinal de pertubação durante a simulação do conversor operando em torno do ponto $P_{\mathrm{o}, 1}$ definido anteriormente.
...
No Cenário 2, foi analisado o impacto do sinal de perturbação $P_{\mathrm{cpl}}$ no conversor Boost, variando a potência da \acrshort{cpl}. Os valores do sinal de perturbação durante a simulação do conversor operando em torno dos pontos $P_{\mathrm{o}, 3}$ e $P_{\mathrm{o}, 4}$ previamente definidos são apresentados na \autoref{fig:boost_power_cpl_variation}.
