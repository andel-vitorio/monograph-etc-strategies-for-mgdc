...
% Linearização - matriz jacobiana
% A definição da matriz jacobiana é apresenta a seguir: 

% \begin{definition}
%   Seja uma função $f: \mathbb{R}^n \rightarrow \mathbb{R}^m$, onde $f$ é composta por funções componentes $f_i: \mathbb{R}^n \rightarrow \mathbb{R}$ para todo $i \in \mathbb{N}_{\leq m}$, dependente das variáveis $x_j$, para todo $j \in \mathbb{N}_{\leq n}$. A matriz jacobiana de $f$, denotada por $J_f(x_1, x_2, ..., x_n)$, é uma matriz $m \times n$ definida como:
